...
func VerifyEmail(c *fiber.Ctx) error {
	...
	if expired.Expires.After(time.Now()) {
		...
		collectionEmailVerificationToken.FindOneAndDelete(c.Context(), filter)
...
func ResendEmailVerificationToken(c *fiber.Ctx) error {
	c.Accepts("application/json")
	userId := new(models.ResendEmailVerificationToken)
	if err := c.BodyParser(userId); err != nil {
		return c.Status(400).SendString(err.Error())
	}
	collectionUser := database.InitDB().Db.Collection("user")
	filter := bson.D{{Key: "_id", Value: userId.ID}}
	var user models.User
	err := collectionUser.FindOne(c.Context(), filter).Decode(&user)
	if err != nil {
		return c.Status(401).SendString("User not found")
	}
	if user.IsVerify {
		return c.Status(400).SendString("This email id is already verified!")
	}
	collectionEmailVerificationToken := database.InitDB().Db.Collection("email_verification_token")

	var alreadyHasToken models.EmailVerificationToken
	filter = bson.D{{Key: "_id", Value: userId.ID}}
	err = collectionEmailVerificationToken.FindOne(c.Context(), filter).Decode(&alreadyHasToken)
	if err != nil {
	...
	newEmailVerificationToken := new(models.EmailVerificationToken)
	newEmailVerificationToken.OwnerID = alreadyHasToken.OwnerID
	hashedOTP, err := utils.Hash(OTP)
	if err != nil {
		return c.Status(500).SendString("b")
	}
	...
	newEmailVerificationToken.Token = hashedOTP

	newEmailVerificationToken.CreatedAt = time.Now()
	newEmailVerificationToken.Expires = newEmailVerificationToken.CreatedAt.Add(1 * time.Minute)

	_, err = collectionEmailVerificationToken.InsertOne(c.Context(), newEmailVerificationToken)

	return c.Status(201).SendString("New OTP has been sent to your registered email accout.")
}
